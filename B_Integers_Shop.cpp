...
void solve(){
    ...
    int a,b,c;
    cin>>a>>b>>c;
    int maxm=b,minm=a,maxi=0,mini=0,midi=0;
    pii mid = make_pair(a,b);
    costs.pb(c);
    ans.pb(c);
    int i=1;
    n--;
    while(n--){
        ...
        if(a<=minm){
            if(a<minm || costs[mini]>costs[i]){
                mini = i;
                minm = a;
            }
        }
        if(b>=maxm){
            if(b>maxm || costs[maxi]>costs[i]){
                maxi = i;
                maxm = b;
            }
        }
        // cout<<midi<<" ";
        if(minm<mid.ff || maxm>mid.ss){
            // cout<<mini<<" "<<maxi;
            ans.pb(costs[mini]+costs[maxi]);
        }
        else{
            ans.pb(min(costs[mini]+costs[maxi],costs[midi]));
        }
        i++;
    }
    rep(j,0,i){
